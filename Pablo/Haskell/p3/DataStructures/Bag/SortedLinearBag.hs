...
occurrences x (Node y oy s) = if (x == y) then oy else occurrences x s

-- borra una ocurrencia de un dato de una bolsa
-- (devuelve la bolsa original si el dato no estaba en la bolsa)
delete :: (Ord a) => a -> Bag a -> Bag a
delete x Empty = Empty
delete x (Node y oy s) = if (x == y) then if (oy == 1) then s else Node y (oy-1) s else Node y oy (delete x s)
...
union (Node x ox s) (Node y oy t)
   ...
   | x < y = Node x ox (union (Node y oy t) s)
...
intersection (Node x ox s) (Node y oy t)
   | x == y = Node y (if ox > oy then oy else ox) (intersection s t)
