...
isEmpty :: Roulette a -> Bool
isEmpty (R q size) = if size > 0 then True else False
...
turn :: Integer ->  Roulette a ->  Roulette a
turn _ (R q 0) = empty
turn 0 (R q size) = R q size
turn n (R q size) 
    | n > 0 = turn (n-1)(R (Q.enqueue(Q.first q) (Q.dequeue q)) size)
    | n < 0 = turn (n+1) (R (Q.enqueue (Q.first q) (Q.dequeue q)) size)
...
rouletteToList :: Roulette a -> [a]
rouletteToList (R _ 0) = []
rouletteToList r@(R q n) = [sign r] ++ rouletteToList(delete r)
...
mapRoulette :: (a -> b) -> Roulette a -> Roulette b
mapRoulette f r@(R q size) = listToRoulette([f (x) | x <- rouletteToList(r)])
