...
sizeValue x = case x of 
                  Just x' -> 1
                  Nothing -> 0

-- | = Exercise d - size
size :: Trie a -> Int
size tri = sizeRec tri 0
  where
    sizeRec :: Trie a -> Int -> Int
    sizeRec Empty n = n
    sizeRec (Node x' d) n
      | sizeValue x' == 1 = foldr (\t acc -> sizeRec t acc) (n+1) (D.values d)
      | otherwise = foldr (\t acc -> sizeRec t acc) n (D.values d)
...
childOf c (Node x d)
  | D.isDefinedAt c d = toTrie (D.valueOf c d)
  | otherwise = Empty

-- | = Exercise g - search
search :: String -> Trie a -> Maybe a
search [] Empty = Nothing
search [] tri@(Node x d) = x
search (w:ws) t = search ws (childOf w t)


-- | = Exercise h - update
update :: Trie a -> Char -> Trie a -> Trie a
update t@(Node x d) c t'@(Node x' d') 
  | isEmpty t = Node Nothing (D.insert c t' D.empty)
  | otherwise = (Node x (D.insert c t' (D.delete c d)))

-- | = Exercise i - insert

insert :: String -> a -> Trie a -> Trie a
insert [] v t@(Node a d) = (Node (Just v) d)
insert (c:cs) v Empty = (Node Nothing (D.insert c (insert cs v (Node Nothing D.empty)) D.empty))
insert (c:cs) v t@(Node a d) 
  |(D.isDefinedAt c d) == True = (update t c (insert cs v (childOf c t)))
  |otherwise = (update t c (insert cs v (Node Nothing D.empty)))
