...
addEdge (WG dic) y z w 
  | not (D.isDefinedAt z dic) = error "addEdge: second vertex is not in the graph"
  | otherwise = case D.valueOf y dic of
                  Just dic' -> WG (D.insert y (addEdge' dic' z w) dic)
                  Nothing -> error "addEdge: first vertex is not in the graph"
    where 
      addEdge' dic' z' w' = D.insert z' w' dic'

successors :: (Ord a, Show a) => WeightedGraph a w -> a -> [(a,w)]
successors (WG dic) x = case D.valueOf x dic of
  Just dic' -> [(v,w) | (v,w) <- D.keysValues dic']
  Nothing -> error "succesors: the vertex is not in the graph"

edges :: (Eq a, Eq w) => WeightedGraph a w -> [WeightedEdge a w]
edges (WG dic) = [(WE u w v) | (u,t) <- D.keysValues dic, (v,w) <- (D.keysValues t)]
